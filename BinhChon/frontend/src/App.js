...
function App() {
  ...
  const connectWallet = useCallback(async () => {
    try {
      ...
      if (CONTRACT_ADDRESS) {
        ...
        const r = Number(await contract.roles(account));
        setRole(r === 1 ? "student" : r === 2 ? "teacher" : "");
      }
    } catch (err) {
  ...
  const createPoll = useCallback(
    async (evt) => {
      ...
      if (!wallet || role !== "teacher") {
        setMessage("Chỉ giảng viên được tạo poll");
        return;
      }
      ...
      <header className="hero">
        <div>
          ...
          <div className="meta">
            ...
            <span>Vai trò: {role ? (role === "teacher" ? "Giảng viên" : "Sinh viên") : "Chưa gán"}</span>
          </div>
      ...
      {canCreateTeacher && (
        <section className="card form-card">
          <div className="form-head">
            <div>
              <p className="eyebrow">Chủ hợp đồng</p>
              <h2>Tạo cuộc bình chọn</h2>
            </div>
            <span className="hint">Chỉ dành cho địa chỉ chủ hợp đồng</span>
          </div>
